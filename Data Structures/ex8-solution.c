...
void processAddChild() {
    ...
    printf("enter name and child: "); scanf("%s%s", name, child);
    addChild(name, child);
}
...
void processFindChildren() {
  ...
  if (p == NULL) printf("not found %s\n", name);
    ...
    printf("found %s with child: ", name);
...
void main() {
  root = makeNode("david");
  printf("command list: Quit Print Find FindChildren Height Count AddChild Store\n");
  while(1){
    char cmd[256]; printf("enter command: "); scanf("%s", cmd); 
